...
export const SnakeGameDifficulty: Record<GameDifficulty, number> = {
	EASY: 500,
	MEDIUM: 450,
	HARD: 400,
};
...
export interface SnakeGameScore {
	energy: number;
	movements: number;
	size: number;
}
	...
	immer((set) => ({
		...
		moveSnake() {
			set((state) => {
				...
				if (ateFood) {
					const minSpeed = state.settings.maxSpeed - 50;
					const progress = newSnake.length / state.settings.energy;
					const dynamicSpeed =
						state.settings.maxSpeed -
						progress * (state.settings.maxSpeed - minSpeed);
...
function initGame(
	...
	const settings: SnakeGameSettings = {
		size,
		maxSpeed: calculateSpeed(size, difficulty),
...
function calculateSpeed(size: number, difficulty = GameDifficulty.MEDIUM) {
